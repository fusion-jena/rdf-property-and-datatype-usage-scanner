package de.uni_jena.cs.fusion.experiment.rdf_float_usage;
...
import de.uni_jena.cs.fusion.experiment.rdf_float_usage.utils.H2Util;
 ...
 * </p>
 */
public class H2CreateDatabases {

	// Queries to generate the database
	// Caution: overwrites existing databases!
	private static final String CREATE_FILE_DATABASE = "CREATE TABLE " + H2Util.FILE_DATABASE
	...
	private static final String CREATE_RESULT_DATABSE = "CREATE TABLE " + H2Util.RESULT_DATABASE
			+ " (FILE_ID BIGINT REFERENCES " + H2Util.FILE_DATABASE
	...
	private static final String CREATE_ERROR_LINE_DATABASE = "CREATE TABLE " + H2Util.ERROR_DATABASE
			+ " (FILE_ID BIGINT REFERENCES " + H2Util.FILE_DATABASE
	...
	public static void main(String[] args) {
		...
		try {
			...
			fillFileOrganisationDatabase(con);
	...
	private static void createTables(Connection con) throws SQLException {
		log.info("Create table " + H2Util.FILE_DATABASE);
		H2Util.executeAndUpdate(con, CREATE_FILE_DATABASE);
		log.info("Table " + H2Util.FILE_DATABASE + " created");

		log.info("Create table " + H2Util.RESULT_DATABASE);
		H2Util.executeAndUpdate(con, CREATE_RESULT_DATABSE);
		log.info("Table " + H2Util.RESULT_DATABASE + " created");

		log.info("Create table " + H2Util.ERROR_DATABASE);
		H2Util.executeAndUpdate(con, CREATE_ERROR_LINE_DATABASE);
		log.info("Table " + H2Util.ERROR_DATABASE + " created");
	}
	 ...
	 * @throws SQLException in case of an invalid SQL command
	 */
	private static void fillFileOrganisationDatabase(Connection con) throws SQLException {
...
//				"html-rdfa" //5167
		...
		for (String file : listFiles) {
			...
			try {
				...
				while (line != null) {
					String query = "INSERT into " + H2Util.FILE_DATABASE + " values (default, '" + line
							+ "', null, null, null)";
